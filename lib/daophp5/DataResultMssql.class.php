...
/**
 * class: DataResultMssql
 * 
 * DataResultMssql handles retrieving row data from the mssql database
 * 
 * see:
 * 
 * 		DataResult
 ...
 *
 */
class DataResultMssql extends DataResult {
	...
	const DATA_RESULT_FORMAT_NUM = MSSQL_NUM;
	const DATA_RESULT_FORMAT_ASSOC = MSSQL_ASSOC;
	const DATA_RESULT_FORMAT_BOTH = MSSQL_BOTH;
	 ...
	 * 
	 * DataResultMssql constructor
	 * 
	 * parameters:
	 * 
	 * 		result - Mssql database result resource
	 */
	 ...
	 * method: getRow
	 * 
	 * returns a row of data from the Mssql database
	...
	public function getRow() {		
		if($row = mssql_fetch_array($this->result, $this->format)) {
	...
	protected function dataSeek($row_num = 0) {
		return mssql_data_seek($this->result, $row_num);
	}
