...
function cParticleEmitter(ctx){

  this.ctx = ctx;

  this.maxParticles = (Math.random() * 150) + 150;
  this.particles = [];
  this.active = true;
  ...
  this.angle = 0;
  this.angleRandom = 180;

  this.gravity = Vector.create( 0.5, 0.3 );

  this.startColour = [ 255,0,0,0.25 ];
  this.startColourRandom = [ 255,255,0,0 ];

  this.finishColour = [ 0,150,255,0 ];
  this.finishColourRandom = [ 255,0,0,0 ];

  this.sharpness = 50;
  this.sharpnessRandom = 40;
  ...
  var width = (ctx.canvas.offsetWidth / 2) - (this.size / 2),
      height = (ctx.canvas.offsetHeight / 2) - (this.size / 2);

  this.position = Vector.create(width, height);
  this.positionRandom = Vector.create(width, height);
  ...
  this.resetWidth = function() {
    this.ctx.clearRect(0, 0, ctx.canvas.offsetWidth, ctx.canvas.offsetHeight);
    this.position = Vector.create( (ctx.canvas.offsetWidth / 2) - this.size, (ctx.canvas.offsetHeight / 2) - this.size);
    this.positionRandom = Vector.create( (ctx.canvas.offsetWidth / 2) - this.size, (ctx.canvas.offsetHeight / 2) - this.size);
  ...
  this.renderParticles = function( context ){
    for( var i = 0, j = this.particleCount; i < j; i++ ){
      ...
      radgrad.addColorStop( 1, 'rgba(0,51,102,0)' ); //Super cool if you change these values (and add more colour stops)
